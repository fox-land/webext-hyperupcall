...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
	...
	if (tab.status == 'complete') {
		...
		if (isPage) {
			...
			var userRepoRegex =
				/http[s]?:\/\/([\da-zA-Z\-]*)?\.github\.io\/?([\da-zA-Z\-\.]*).*/i
			var userRepo = userRepoRegex.exec(tab.url)
			var userRepoObj = {
				username: userRepo[1],
				repo: userRepo[2] ? userRepo[2] : userRepo[1] + '.github.io',
			}
			// console.log(userRepoObj);

			// Check localstorage if we already have this repo's info
			var storageKey = (
				userRepoObj.username +
				'_' +
				userRepoObj.repo.replace(/(\.|\/)/g, '_')
			).toLowerCase()
