...
extension.events.checkActiveElement = function () {
	if (
		...
		['EMBED', 'INPUT', 'OBJECT', 'TEXTAREA', 'IFRAME'].includes(
	...
	if (extension.ui.classList.contains(extension.prefix + '--visible')) {
		for (var key in extension.events.features) {
			...
			if (shortcut) {
				...
				if (
					(extension.events.data.alt === shortcut.alt ||
						shortcut.hasOwnProperty('alt') === false) &&
					(extension.events.data.ctrl === shortcut.ctrl ||
						shortcut.hasOwnProperty('ctrl') === false) &&
					(extension.events.data.shift === shortcut.shift ||
						shortcut.hasOwnProperty('shift') === false) &&
					(extension.events.data.wheel === shortcut.wheel ||
						shortcut.hasOwnProperty('wheel') === false)
				) {
...
extension.events.mouse.mousedown = function (event) {
	...
	if (extension.storage.items.hidden !== true) {
		if (extension.ui.hover(extension.ui.drag.getBoundingClientRect())) {
			...
			extension.events.clickDrag.x =
				event.clientX -
				extension.ui.offsetLeft -
				extension.ui.surface.offsetLeft -
				extension.ui.surface.offsetWidth
			extension.events.clickDrag.y =
				event.clientY - extension.ui.offsetTop - extension.ui.surface.offsetTop
		...
		if (extension.ui.hover(extension.ui.resize.getBoundingClientRect())) {
			...
			extension.events.clickResize.x =
				event.clientX -
				extension.ui.offsetLeft -
				extension.ui.surface.offsetLeft -
				extension.ui.surface.offsetWidth
			extension.events.clickResize.y =
				event.clientY -
				extension.ui.offsetTop -
				extension.ui.surface.offsetTop -
				extension.ui.surface.offsetHeight
	...
	for (var i = 0, l = mutationList.length; i < l; i++) {
		...
		if (mutation.type === 'childList') {
			for (var j = 0, k = mutation.addedNodes.length; j < k; j++) {
				extension.observer.parseChildren(
...
extension.ui.actions.toggle = function () {
	extension.ui.surface.classList.toggle(
		extension.prefix + '__surface--collapsed',
	)

	chrome.storage.local.set({
		hidden: extension.ui.surface.classList.contains(
			extension.prefix + '__surface--collapsed',
		),
	})
}
...
extension.ui.actions.dragAndDrop = function (event) {
	...
	x = Math.min(
		x,
		extension.ui.offsetWidth - extension.ui.surface.offsetWidth - 8,
	)
	y = Math.min(
		y,
		extension.ui.offsetHeight - extension.ui.surface.offsetHeight - 8,
	)
...
extension.ui.actions.resize = function (event) {
	...
	width = Math.min(
		width,
		extension.ui.offsetWidth - extension.ui.surface.offsetLeft - 8,
	)
	height = Math.min(
		height,
		extension.ui.offsetHeight - extension.ui.surface.offsetTop - 8,
	)
...
extension.ui.create = function () {
	...
	this.toggle.className =
		extension.prefix + '__button ' + extension.prefix + '__toggle'
	this.drag.className =
		extension.prefix + '__button ' + extension.prefix + '__drag-and-drop'
	this.resize.className =
		extension.prefix + '__button ' + extension.prefix + '__resize'
	...
	this.surface.update = function () {
		...
		if (extension.storage.items.size) {
			...
			width = Math.min(
				width,
				extension.ui.offsetWidth - Math.max(8, this.offsetLeft) - 8,
			)
			height = Math.min(
				height,
				extension.ui.offsetHeight - Math.max(8, this.offsetTop) - 8,
			)
	...
	if (video) {
		if (this.classList.contains(extension.prefix + '--busy') === false) {
			if (extension.ui.hover(this.surface.getBoundingClientRect())) {
				...
				if (
					extension.storage.items.hidden !== true &&
					extension.ui.hover(this.drag.getBoundingClientRect())
				) {
				...
				if (
					extension.storage.items.hidden !== true &&
					extension.ui.hover(this.resize.getBoundingClientRect())
				) {
...
extension.message.sent('get-tab-url', function (response) {
	...
	extension.storage.import(function () {
		if (
			!extension.storage.items.domains ||
			extension.storage.items.domains[extension.hostname] !== false
		) {
			extension.enable()
		}
	})
...
document.addEventListener('fullscreenchange', function () {
	if (
		...
		extension.storage.items.hide_in_fullscreen === true
	) {
