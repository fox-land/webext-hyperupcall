...
function debug(/** @type {string} */ msg) {
	if (true) console.log(`github-issue-link-status: ${msg}`)
}
...
function getIcon(state, type, isDraft = false) {
	...
	if (iconName === 'draftpullrequest') {
		return icons.draftpullrequest.replace(
			'<svg',
			'<svg class="octicon color-text-tertiary"',
		)
	}
...
async function apply() {
	...
	if (response.status === 401) {
		console.error(
			'GitHub Issue Link Status: The token was not accepted, it’s likely expired',
		)
		return
	}
...
async function init() {
	chrome.storage.sync.get(['personalAccessToken'], async (result) => {
		...
		if (token) {
			...
			console.error(
				'GitHub Issue Link Status: you will need to set a token in the options for this extension to work.',
			)
		}
	})
}
