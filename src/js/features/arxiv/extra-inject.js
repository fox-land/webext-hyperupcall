...
app.insertTitle = function (id, type, title, newTitle) {
	...
	if (app.title_rename_count > app.title_rename_max) {
		// console.log(
		// 	app.name,
		// 	'Warning: Title insertion cancelled. Too many title changes',
		// )
		return
	}
	...
	if (elHtmls.length !== 0) {
		...
		if (elHtml.firstChild !== null) {
			...
			// console.log(
			// 	app.name,
			// 	'Modify <html> tag by inserting before first child.',
			// )
		} else {
	...
	chrome.storage.sync.get(
		...
		function (res) {
			...
			if (elULs.length === 0) {
				// console.log(
				// 	app.name,
				// 	"Error: Items selected by '.full-text > ul' not found",
				// )
				return
			}

			const linkText = '[arXiv-extra] Link'
			var elUL = elULs[0]
			var elLI = document.createElement('li')
			var elA = document.createElement('a')
			var directURL = 'https://arxiv.org/pdf/' + id + '.pdf'
			elA.innerText = linkText
