...
document.addEventListener('ui-create', function (event) {
	...
	time_container.appendChild(
		document.createTextNode(extension.locale.get('time')),
	)
	duration_container.appendChild(
		document.createTextNode(extension.locale.get('duration')),
	)
	frame_container.appendChild(
		document.createTextNode(extension.locale.get('frame')),
	)
	framerate_container.appendChild(
		document.createTextNode(extension.locale.get('framerate')),
	)
...
document.addEventListener('ui-update', function () {
	...
	extension.ui.duration.textContent =
		extension.videos.active.duration.toFixed(2)
	extension.ui.frame.textContent = Math.floor(
		extension.videos.active.currentTime * framerate,
	)
...
document.addEventListener('ui-styles', function () {
	...
	if (storage.background_color) {
		if (storage.hasOwnProperty('opacity')) {
			extension.ui.surface.style.setProperty(
				'background-color',
				'rgba(' +
					storage.background_color.join(',') +
					',' +
					storage.opacity +
					')',
				'important',
			...
			extension.ui.surface.style.setProperty(
				'background-color',
				'rgba(' + storage.background_color.join(',') + ',0.8)',
				'important',
			)
		}
	}
	...
	if (storage.text_color) {
		extension.ui.surface.style.setProperty(
			'color',
			'rgb(' + storage.text_color.join(',') + ')',
			'important',
		)
		extension.ui.surface.style.setProperty(
			'--fbf-color',
			storage.text_color.join(','),
			'important',
		)
	}
